...
| Field | Value |
|---|---|
| Blueprint label | `thm:Tphi-compact` |
| Blueprint kind  | theorem |
| Lean target     | `PeterWeyl.convOp_isCompactOperator` |
| Source location | `PeterWeylComp/Convolution.lean:154` |
...
`def:Tphi`, `thm:haar-rightInv`, `thm:haar-isProb`

### Definition of done

- [ ] No `sorry` in `PeterWeyl.convOp_isCompactOperator` (or any helper introduced for it).
- [ ] Add `\leanok` to `thm:Tphi-compact` in `blueprint/src/content_comp.tex`.
