...
\begin{corollary}[${k[G]}$ is a semisimple ring]
  ...
  \lean{MonoidAlgebra.instIsSemisimpleRing}
  \mathlibok
  \leanok
  \uses{thm:maschke}
...
\begin{proof}\leanok\uses{thm:avg-char}
...
\begin{lemma}[$\chi_{k[G]}$ is supported at $1$]
  \label{lem:char-regular}
  \lean{FDRep.character_leftRegular}
  % [~ML] Folklore; not packaged as a single lemma in Mathlib but is one
  %       short calculation: trace of a permutation matrix counts fixed
  %       points and left translation by g ≠ 1 is fixed-point free on G.
...
\begin{proof}
  \uses{def:character}
  In the basis $\{\delta_h : h \in G\}$, the matrix of left translation by
  $g$ is the permutation matrix for $h \mapsto gh$. Its trace counts
  fixed points: $h = gh$ iff $g = 1$, in which case all $|G|$ basis vectors
  are fixed.
\end{proof}
...
\begin{lemma}[Finite-dim division algebra over alg-closed $k$ equals $k$]
  ...
  \lean{IsAlgClosed.divisionRing_algEquiv}
  % [NEW] Standard but not packaged with this name in Mathlib. Easy:
  %       any element of D is algebraic over k (D is finite-dim), hence
  %       satisfies a non-zero polynomial; over alg-closed k the minimal
  %       polynomial of any element splits with linear factors, but
  %       irreducibility of the minimal polynomial in a division algebra
  %       forces degree 1, i.e. the element is in k.
  \uses{def:standing-hypotheses}
...
\begin{proof}
  ...
  as $k$-algebras, and apply the matrix-algebra functor to get
  $\mathrm{Mat}_{d_i}(D_i) \simeq \mathrm{Mat}_{d_i}(k)$. Compose.
\end{proof}
...
\begin{proof}
  ...
  $\dim_k\,k[G] = |G|$; the right side is
  $\sum_i \dim_k \mathrm{Mat}_{d_i}(k) = \sum_i d_i^2$.
\end{proof}

\section{Module-form Peter–Weyl: ${k[G] \cong \bigoplus_i V_i^{\oplus d_i}}$}
...
\texttt{FDRep.character\_leftRegular} (Lemma~\ref{lem:char-regular}),
...
There are only four:
\begin{enumerate}
  \item \texttt{IsAlgClosed.divisionRing\_algEquiv} — every finite-dimensional
    division algebra over an algebraically closed field equals the field
    (Lemma~\ref{lem:divalg-of-isAlgClosed}). The proof is a 5-line argument.
